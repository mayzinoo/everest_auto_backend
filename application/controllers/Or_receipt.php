...
class Or_receipt extends CI_Controller
{
    ...
    function index()
    {
        ...
        {           
            $data["orreceipt"] =$this->db->query("SELECT * FROM or_tbl WHERE type='gst'");
            $data['content']="gstinvoice/or-receipt/index";
    ...
    function create_form()
    {
        ...
        {    
            ...
            $data['content']="gstinvoice/or-receipt/create_form";
            $this->load->view("template",$data);
        }else{
          redirect('/');
        }
    ...
    function edit_form()
    {
        ...
        if($this->session->userdata("email") && $this->session->userdata("password") && $role=="admin" || $role=="sale")
        {
            ...
            $data['content']="gstinvoice/or-receipt/edit_form";
            $this->load->view("template",$data);
        }else{
            redirect('/');
        }
    ...
    function orreceipt_pdf()
    {
        $id=$this->uri->segment(3);
        $data["orreceipt"]=$this->db->query("SELECT * FROM or_tbl WHERE id='$id'")->row();

        $pdf_view=$this->load->view('gstinvoice/or-receipt/orreceiptpdf',$data, true);
        $pdfFilePath = 'GST Offical Receipt.pdf';
