...
class Gmsinvoice extends CI_Controller
{
    ...
    function index()
    {
        ...
        {           
            $data["gmsinvoice"] =$this->db->query("SELECT * FROM gstinvoice WHERE type='gms'");
            $data['content']="gmsinvoice/index";
    ...
    function create_form()
    {
        ...
        {    
            ...
            $data['content']="gmsinvoice/create_form";
            $this->load->view("template",$data);
        }else{
          redirect('/');
        }
    ...
    function edit_form()
    {
        ...
        if($this->session->userdata("email") && $this->session->userdata("password") && $role=="admin" || $role=="sale")
        {
            ...
            $data['companylist']=$this->Gstinvoice_model->getcompanylist();
            $data["gmsinvoice"] =$this->db->query("SELECT * FROM gstinvoice WHERE id='$id'")->row();
            $data['content']="gmsinvoice/edit_form";
            $this->load->view("template",$data);
        }else{
            redirect('/');
        }
