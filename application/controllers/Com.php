...
class Com extends CI_Controller
{
    ...
    function index()
    {
        ...
        {           
            $data["com"] =$this->db->query("SELECT * FROM com WHERE type='gst'");
            $data['content']="gstinvoice/com/index";
    ...
    function create_form()
    {
        ...
        {    
            ...
            $data['content']="gstinvoice/com/create_form";
            $this->load->view("template",$data);
        }else{
          redirect('/');
        }
    ...
    function edit_form()
    {
        ...
        if($this->session->userdata("email") && $this->session->userdata("password") && $role=="admin" || $role=="sale")
        {
            ...
            $data['customerlist']=$this->Gstinvoice_model->gethirerlist();
            $data['vehilist']=$this->Gstinvoice_model->gstvehilist();
            $data["com"] =$this->db->query("SELECT * FROM com WHERE id='$id'")->row();
            $data['content']="gstinvoice/com/edit_form";
            $this->load->view("template",$data);
        }else{
            redirect('/');
        }
    ...
    function com_pdf()
    {
        ...
        $pdf_view=$this->load->view('gstinvoice/com/comformpdf',$data, true);
        $pdfFilePath = 'COM.pdf';
