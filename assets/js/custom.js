...
function costdetailcloneform()
{
    
     var clone=$( ".costdetail:last-child" ).clone().appendTo("#cost_detail" );
      clone.find("input[name='costtype[]']").val("");
      clone.find("option:selected").prop("selected", false)
      clone.find("input[name='description[]']").val("");
      clone.find("input[name='vendor[]']").val(""); 
      clone.find("input[name='refdate[]']").val("");
      clone.find("input[name='refno[]']").val("");
      clone.find("input[name='amount[]']").val("");    
      clone.find("input[name='gst[]']").val("");
      clone.find("input[name='costtotal[]']").val(""); 
      window.scrollBy(0, 70);    
    
}

function removercostdetail(event)
{
    var target = $(event.target);
    var cl=$(".costdetail").length;
    if(cl==1)
    {
...
function commissioncloneform()
{
 
     var clone=$( ".commission:last-child" ).clone().appendTo("#commissdetail" );
     
      clone.find("option:selected").prop("selected", false)  
      clone.find("input[name='commi_des[]']").val("");    
      clone.find("input[name='commission[]']").val("");
      clone.find("input[name='paiddate[]']").val(""); 
}
function removercommission(event)
{
    var target = $(event.target);
    var cl=$(".commission").length;
    if(cl==1)
    {
    alert("You can not remove");
    }
...
function floorstockcloneform()
{
   
     var clone=$( ".floorstock:last-child" ).clone().appendTo("#floorstockinterest" );
      clone.find("input[name='loanamt[]']").val("");
      clone.find("input[name='startdate[]']").val("");
      clone.find("input[name='enddate[]']").val(""); 
      clone.find("input[name='interestamt[]']").val("");
      clone.find("input[name='type[]']").val("");
  
}
function removerfloorstock(event)
{
    var target = $(event.target);
    var cl=$(".floorstock").length;
    if(cl==1)
    {
    alert("You can not remove");
    }
...
function parkingfeecloneform(arg)
{
    arg.preventDefault();
     var cloneCount = 1;   
     var clone=$( ".parking:last-child" ).clone().appendTo("#parkingfee" );
      clone.find("input[name='rate[]']").val("");
      clone.find("input[name='parking_start_date[]']").val("");
      clone.find("input[name='parking_end_date[]']").val(""); 
      clone.find("input[name='total_parkingfee[]']").val("");
  
}
function removerparking(event)
{
    var target = $(event.target);
    var cl=$(".parking").length;
    if(cl==1)
    {
    alert("You can not remove");
    }
...
function brokerpayoutcloneform()
{
  
     var clone=$( ".brokerpayout:last-child" ).clone().appendTo("#brokerpayoutdetail" );
      clone.find("input[name='broker[]']").val("");
      clone.find("input[name='brokercode[]']").val("");
      clone.find("input[name='grossprofit[]']").val("");
      clone.find("input[name='cif[]']").val(""); 
      clone.find("input[name='broker_paid_date[]']").val("");
      clone.find("input[name='payout[]']").val("");
      clone.find("input[name='percent[]']").val("");
}
function removerbrokerpayout(event)
{
    var target = $(event.target);
    var cl=$(".brokerpayout").length;
    if(cl==1)
    {
    alert("You can not remove");
    }
...
function inclusivecloneform(arg)
{
    arg.preventDefault();
     var cloneCount = 1;   
     var clone=$( ".inclusive:last-child" ).clone().appendTo("#inclusiveitem" );
      clone.find("input[name='item[]']").val("");
      clone.find("input[name='name[]']").val("");
      clone.find("input[name='amt[]']").val(""); 
  
}
function removerinclusive(event)
{
    var target = $(event.target);
    var cl=$(".inclusive").length;
    if(cl==1)
    {
    alert("You can not remove");
    }
...
function costsheetreportclone()
{
    //arg.preventDefault();
        var clone=$( ".costsheetdes:last-child" ).clone();
        
        clone.appendTo( "#costsheetdescription" );     
        clone.find("input[name='des1[]']").val("");
        clone.find("input[name='des2[]']").val("");
        clone.find("input[name='des3[]']").val("");
        clone.find("input[name='date[]']").val("");
        clone.find("input[name='profit[]']").val("");
        clone.find("input[name='costing[]']").val("");
        clone.find("input[name='total[]']").val("");
        window.scrollBy(0, 70);
}
function removerdescription(event)
{
    var target = $(event.target);
    
    var cl=$(".costsheetdes").length;
    if(cl==1)
    {
    alert("You can not remove");
    }
...
function quotationcloneform()
{
    
     var clone=$( ".quotation:last-child" ).clone().appendTo("#quotation_valid" );
      
      clone.find("input[name='insurance[]']").val("");
      clone.find("input[name='premium[]']").val(""); 
      clone.find("input[name='excess[]']").val("");
      clone.find("input[name='windscr[]']").val("");
      clone.find("input[name='remark[]']").val("");
      window.scrollBy(0, 70); 
}

function removerquotation(event)
{
    var target = $(event.target);
    var cl=$(".quotation").length;
    if(cl==1)
    {
    alert("You can not remove");
    }
...
// Add new vehicles
$("#new-vehicles").on("click", function() {
        $("#modal-new-vehicles").modal("show");
    });
...
$(".edit-business").on("click", function() {
        ...
        $.ajax({                
                url : base_url+"business/showbusinessdata/"+id,
                type: "GET",
                dataType: "JSON",
                success : function(e)
                {
                  ...
                  if(e.photo)
                    {
                        ...
                        $('#photo-preview div').append('<input type="checkbox" name="remove_photo" value="'+e.photo+'"/></br> Remove photo when saving'); // remove photo
...
$(".edit-fueltype").on("click", function() {
    let id = $(this).data("id");
        ...
        $.ajax({                
            url : base_url+"functions/showfueltype/"+id,
            type: "GET",
            dataType: "JSON",
            success : function(e)
            {
...
$(".edit-bodytype").on("click", function() {
    let id = $(this).data("id");
        ...
        $.ajax({                
            url : base_url+"functions/showbodytype/"+id,
            type: "GET",
            dataType: "JSON",
            success : function(e)
            {
...
$(".edit-brands").on("click", function() {
    ...
    $.ajax({                
            url : base_url+"brands/showbrandsdata/"+id,
            type: "GET",
            dataType: "JSON",
            success : function(e)
            {
...
$(".edit-howitworks").on("click", function() {
    ...
    let id = $(this).data("id");         
    $.ajax({                
            url : base_url+"howitwork/showhowitworksdata/"+id,
            type: "GET",
            dataType: "JSON",
            success : function(e)
            {
...
// FAQ
    $(".edit-faq").on("click", function() {
        let id = $(this).data("id");
            ...
            $.ajax({                
                url : base_url+"faq/showfaqdata/"+id,
                type: "GET",
                dataType: "JSON",
                success : function(e)
                {
...
// Features
    $(".edit-feature").on("click", function() {
        let id = $(this).data("id");
            ...
            $.ajax({                
                url : base_url+"features/showfeaturedata/"+id,
                type: "GET",
                dataType: "JSON",
                success : function(e)
                {
...
// Services
    $(".edit-services").on("click", function() {
        let id = $(this).data("id");
            ...
            $.ajax({                
                url : base_url+"services/showservicesdata/"+id,
                type: "GET",
                dataType: "JSON",
                success : function(e)
                {
...
/*edit Vehicles*/    
   let key = 0;      
    $(".edit-vehicles2").on("click", function() {
        ...
        $.ajax({
                type: "POST",
                url : base_url+"vehicles/showvehiclesdata",
                data : data,
                success : function(e)
                {                  
                  ...
                  $.each(json_response, function(i, data) {
                   ...
                   var cid=data.id;
                    ...
                    $.ajax({
                            type: "post",
                            url : base_url+"vehicles/getdurationlist/",
                            data: 'cid=' + cid,
...
$(".edit-galleryImage").on("click", function() {
    ...
    $.ajax({                
            url : base_url+"vehicles/showgalleryImage/"+id,
            type: "GET",
            dataType: "JSON",
            success : function(e)
            {
              ...
              if(e.photos)
                {
                    $('#label-photo').text('Change Photo'); // label photo upload
                    $('#photo-preview div').html('<img src="'+base_url+'upload/files/'+e.photos+'" class="galleryimg">'); // show photo
                    $('#photo-preview div').append('<br/><input type="checkbox" name="remove_photo" value="'+e.photos+'"/> Remove photo when saving'); // remove photo
...
$(".edit-coverImage").on("click", function() {
    ...
    $.ajax({                
            url : base_url+"vehicles/showvehiclesphotodata/"+id,
            type: "GET",
            dataType: "JSON",
            success : function(e)
            {
              ...
              if(e.photo)
                {
                    $('#label-photo').text('Change Photo'); // label photo upload
                    $('#photo-preview div').html('<img src="'+base_url+'upload/files/'+e.photo+'" class="galleryimg">'); // show photo
                    $('#photo-preview div').append('<br/><input type="checkbox" name="remove_photo" value="'+e.photo+'"/> Remove photo when saving'); // remove photo
...
$(document).on('click', '.booking_approve', function(){
   
    let id = $(this).data("id");  
    $.ajax({                
            url : base_url+"booking/showuserbooking/"+id,
            type: "GET",
            dataType: "JSON",
            success : function(e)
            {
...
$(document).on('click', '#bookingapprovestatus', function(){

        var formData = new FormData($('#update_approve')[0]);
        $.ajax({
                    url : base_url+"booking/update_bookingapprove",
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
...
// Add Users
    $(".edit-users").on("click", function() {
        let id = $(this).data("id");
            ...
            $.ajax({                
                url : base_url+"users/showusersdata/"+id,
                type: "GET",
                dataType: "JSON",
                success : function(e)
                {
